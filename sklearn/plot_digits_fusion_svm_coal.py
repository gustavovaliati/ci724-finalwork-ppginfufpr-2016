...
def GridSearch(X_train, y_train):
        ...
        # srv = svm.LinearSVC(probability=True)
...
if(args['load_file']):
    ...
    train_dataset  = {"images" : [] , "labels": [], "class_name": []}
    train_dataset["images"] = data[:n_samples // 2]
    train_dataset["labels"] = digits.target[:n_samples // 2]
    train_dataset["class_name"] = digits.target[:n_samples // 2]
...
if(args['need_test']):
    ...
    test_dataset = {"images" : [] , "labels": [], "class_name": []}
    test_dataset["images"] = data[n_samples // 2:]
    test_dataset["labels"] = digits.target[n_samples // 2:]
    test_dataset["class_name"] = digits.target[n_samples // 2:]
...
if args['need_train'] and 'classifier' not in locals():
    ...
    classifier = GridSearch(train_data, train_dataset["labels"])
