...
#     build_hdf5_image_dataset(args['dataset_origin_train'], image_shape=(256, 256), mode='folder', output_path=dataset_train_hdf5, categorical_labels=True, normalize=True)

dataset_hdf5_file = Path(dataset_test_hdf5)
if not dataset_hdf5_file.exists():
    print("Creating",dataset_test_hdf5)
    build_hdf5_image_dataset(args['dataset_origin_test'], image_shape=(256, 256), mode='folder', output_path=dataset_test_hdf5, categorical_labels=True, normalize=True)
...
with sess.as_default():
    ...
    loss = fully_connected(pool5_7_7, num_classes, activation='softmax', restore='False')
    network = regression(loss, optimizer='momentum',
                         loss='categorical_crossentropy',
                         learning_rate=0.001)
    model = tflearn.DNN(network, checkpoint_path='coal_tflearn_inception_checkpoint',
                                            max_checkpoints=3, tensorboard_verbose=2)
    # model.fit(X, Y, n_epoch=1000, validation_set=0.1, shuffle=True,
    #           show_metric=True, batch_size=64, snapshot_step=200,
    #           snapshot_epoch=False, run_id='grv')
    # model.save('coal70_inception.model')
    model.load('coal_tflearn_inception_checkpoint-28000')
    # print(model.evaluate(X_test, Y_test))

    # x = tf.placeholder(shape=(256, 256, 3), dtype=tf.float32)
    x = tf.placeholder(dtype=tf.float32)
    print(inception_5b_pool_1_1.eval(feed_dict={x: X_test[0]},session=sess))
